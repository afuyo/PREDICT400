...
def wholesale(copies):
    ...
    shippingFirst = 3.0
    shippingOthers = 0.75
    wholesale_cost= (copies*cost*discount) + shippingFirst+(shippingOthers*(copies-1))
    return wholesale_cost
    
print (60.0*24.95)*0.6+3.0+59.0*0.75
print wholesale(60)
...
#time_run = 2.0*((60.0*8.0)+15.0)+3.0*((60.0*7.0)+12.0)
time_run= float(2*((60*8)+15)+3*((60*7)+12))
start_sec = ((6*60)+52)*60
total=time_run+start_sec
minutes,s = divmod(total,60)
h,m = divmod(minutes,60)
s=int(s)
m=int(m)
h=int(h)
print "I get home at %d:%d:%d"%(h,m,s)